...
class App(QtWidgets.QMainWindow):
    def __init__(self, motor):
        ...
        data.start()
        self.motor.config_watchdog(True, 2.0)
            ...
            if t1 - t0 > i / f:
                ...
                if self.power == 0.0:
                    ...
                    self.motor.odrv0.axis0.controller.input_torque = \
                        - self.motor._sign() * (abs(torque) + resisting_torque) * self.motor._reduction_ratio
                ...
                if abs(vel) < vel_min:
                    torque = self.power / (vel_min * 2 * np.pi / 60)
                else:
                    torque = self.power / (abs(vel) * 2 * np.pi / 60)
    ...
    def _data(self):
        ...
        while self.run:
            ...
            self.motor.save_data_to_file(f'XP/gui_{rd}', self.ui.instruction_spinBox.value())
            self.motor.odrv0.axis0.watchdog_feed()
